...
@section('content')
{!! Form::open(['method' => 'PUT', 'route' => ['user.email-template.update', $id]]) !!}
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            {!!Form::label('email_type', 'Message Type')!!}: 
            <span class="badge badge-success">{!! title($email_type) !!}</span>
        </div>
        <div class="form-group">
            {!!Form::label('subject', 'Subject')!!} <span class="text-danger">*</span>
            {!! Form::text('subject', $subject, ['class'=>'form-control', 'placeholder'=>'Subject Name', 'required'=>true]) !!}
            {!! $errors->first('subject', '<p class="text-danger">:message</p>') !!}
        </div>
        <div class="form-group">
            {!!Form::label('body', 'Email Template')!!} <span class="text-danger">*</span>
            {!! Form::textarea('body', $body, ['class'=>'form-control summernote', 'placeholder'=>'Email Template', 'rows'=>'5', 'required'=>true]) !!}
            {!! $errors->first('body', '<p class="text-danger">:message</p>') !!}
        </div>
        <div class="form-group">
            {!!Form::label('sms_message', 'SMS Template')!!} <span class="text-danger">*</span>
            {!! Form::textarea('sms_message', $sms_message, ['class'=>'form-control', 'placeholder'=>'SMS Template', 'rows'=>'5', 'required'=>true]) !!}
    ...
    <div class="col-sm-4 border">
        ...
        <?php $shortcodes = [
            ...
            'CUSTOMER_PHONE' => 'Phone of Customer',
            'PRICE' => 'Price of Service',
            'DEVICE' => 'Device Name',
            'MODEL' => 'Model/Item name',
            'STORE_NAME' => 'Name of Store',
            'STORE_DETAIL' => 'Detial of Store',
        ];
        ?>
    ...
    <div class="form-group text-center">
        ...
        &nbsp; &nbsp;
        {{ back_button(route('user.email-template.index')) }}
    </div>
{!! Form::close() !!}
@endsection
